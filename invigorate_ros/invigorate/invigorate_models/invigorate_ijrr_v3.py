...
In this version, we simplify the question grounding process by a hard
linguistic observation model.

To do so, we firstly match questions to objects using a visual grounding model
e.g. MAttNet, and with one-step belief tracking, we can estimate the probability
of each object being matched to each question p(m_i=1|a^q), just as in Invigorate_IJRR.

Then we assign a matching label, which is selected from {-1, 0, 1} to each
(object, question) pair. -1 means not matched, 1 means matched, and 0 means
unknown. The observation model here is:
...
s_i=1, a^q!=i           0               1
s_i=0, a^q!=i           e               1-e
s_i=1, unknown          0.5             0.5
s_i=0, unknown          0.5             0.5
-----------------------------------------------

The matching label is assigned with predefined threshold. For example, we can
set a low thresh 0.25 and high thresh 0.75, which means if the matching prob
is in (0.25, 0.75), the label will be 0.

This model can be seen as another direct extension of the original RSS version.
However, there might be some objects that are always unknown to all questions,
and in this case, the robot cannot disambiguate them. Hence, we propose to
combine the asking with pointing and asking without pointing:
...
class InvigorateIJRRV3(object):
    ...
    def match_question_to_object(self, img, bboxes, classes, questions):
        ...
        q_match = np.zeros(q_matching_prob.shape).astype(np.int32)
        q_match[q_matching_prob > Q_THRESH_HIGH] = 1
        q_match[q_matching_prob < Q_THRESH_LOW] = -1

        print('--------------------------------------------------------')
        logger.info('Object & Question Matching: \n{}'.format(q_match))
        print('--------------------------------------------------------')

        self.belief['q_match'] = q_match
    ...
    def estimate_state_with_user_answer(self, action, answer):
        ...
        is_matched = self.belief['q_match'][target_idx]
        ...
        if response is not None:
            ...
            for (det_ind, pool_ind) in det_to_pool.items():
    ...
    def _planning_with_macro(self, infos):
        ...
        def forward_belief(belief, is_matched, ans):
        ...
        def estimate_q_vec(belief, current_d):
            if current_d == planning_depth - 1:
                ...
                for i, match_prob in enumerate(belief["q_match"]):
                    if current_d == 0:
                        print(i)
        ...
        belief["q_match"] = torch.from_numpy(belief["q_match"])
        belief["infos"] = infos
