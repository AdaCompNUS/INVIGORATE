#!/usr/bin/env python
'''
In this version, we implement several improvements:

    1. The asked question together with the answer can also be used in latter
        visual grounding steps.

        For example, if the robot asks a question, 'Do you mean the cup on the
        right?' without pointing to any object, meaning that this question is
        not instance-specific, and the user answered 'No', the visual grounding
        in the latter step can also utilize the fact that **all cups on the right
        must not be the target**.

        Therefore, we introduce 'negative expressions' in this version, and
        store all negative expressions collected during the interaction for
        latter visual grounding steps.

        Formally, for a target i, we use m^p to represent the matching relationship
        with positive questions and m^n to represent the matching relationship
        with negative questions. Since we already get information from the human
        that m^p=1 and m^n=0, in each step, we have the belief tracking as follows:

            P(s_i | m^p=1, m^n=0) \propto P(m^p, m^n | s_i)
                = P(m^p=1 | s_i) * P(m^n=0 | s_i)
                = P(m^p=1 | s_i) * (1 - P(m^n=1 | s_i))

        where we can utilize the visual grounding observation model to compute both:

            P(m^p=1 | s_i) and P(m^n=1 | s_i)

    2. To prevent asking repetitive questions, we implement two features:

        1) we collect all asked questions, and in each step, if the robot intends to
            ask a question, it will search in the record, and see if there will be a
            same question (here we use METEOR score to measure the similarity). If so,
            the robot will not ask the question and answer by itself.

        2) during planning, there might be questions with the same costs. In this case,
            we will check them one by one. If there is an asked question, it will select
            this question, and answer by itself.
...
from libraries.utils.expr_processor import ExprssionProcessor
...
PENALTY_FOR_ASKING_WITH_POINTING = -3
...
class InvigorateIJRRV5(object):
    ...
    def estimate_state_with_img(self, img, expr):
        self.img = img
        if expr not in self.pos_expr:
            self.pos_expr.append(expr)
        self.subject = self.expr_processor.find_subject(expr)
    ...
    def  multistep_grounding(self, img, bboxes, classes, det_to_pool):
        ...
        pos_grounding_scores = \
            [self._vis_ground_client.ground(img, bboxes, e, classes)
             for e in self.pos_expr]

        neg_grounding_scores = \
            [self._vis_ground_client.ground(img, bboxes, e, classes)
             for e in self.neg_expr]

        logger.info('Perceive_img: mattnet grounding finished')

        for i, g_score in enumerate(pos_grounding_scores):
            logger.info("grounding scores against positive expression '{}' is".format(self.pos_expr[i]))
            logger.info(g_score)
            # multistep state estimation
            self._multistep_p_cand_update(g_score, det_to_pool, is_pos=True)

        for i, g_score in enumerate(neg_grounding_scores):
            logger.info("grounding scores against negative expression '{}' is".format(self.neg_expr[i]))
            logger.info(g_score)
            # multistep state estimation
            self._multistep_p_cand_update(g_score, det_to_pool, is_pos=False)
    ...
    def match_question_to_object(self, img, bboxes, classes, questions):
        ...
        self.belief['q_matching_prob'] = np.concatenate(
            (object_specific_q_match, self.belief['q_matching_prob']), axis=0)
        self.belief['questions'] = object_specific_questions + self.belief['questions']
    ...
    def estimate_state_with_user_answer(self, action, answer):
        ...
        if target_idx >= obj_num:
            ...
            # object instance
            if response:
                self.pos_expr.append(self.belief['questions'][target_idx])
            else:
                self.neg_expr.append(self.belief['questions'][target_idx])
        ...
        if clue != "":
            ...
            bboxes = self.belief['bboxes']
            classes = self.belief['classes']
            cls_scores_list = self.belief['cls_scores_list']
            ...
            self.pos_expr.append(clue)

        # update the belief using the response
        if response is not None:

            for i, (det_ind, pool_ind) in enumerate(det_to_pool.items()):
                # llh = LINGUISTIC_OBSERVATION_MODELS[
                #     'o={:d},m={:d}'.format(response, is_matched[det_ind])]
                # self.object_pool[pool_ind]["cand_belief"].update_with_likelihood(
                #     llh, enable_low_thresh=False)
    ...
    def search_answer(self, question):
        ...
        pos_match_scores = \
            self.expr_processor.semantic_similarity(question, asked_pos)
        neg_match_scores = \
            self.expr_processor.semantic_similarity(question, asked_neg)

        assert not (max(pos_match_scores) > SAME_EXPRESSION_THRESH
                    and max(neg_match_scores) > SAME_EXPRESSION_THRESH)

        answer = None
        if max(pos_match_scores) > SAME_EXPRESSION_THRESH:
            answer = 'yes'
        elif max(neg_match_scores) > SAME_EXPRESSION_THRESH:
            answer = 'no'
        return answer
    ...
    def _planning_with_macro(self, infos):
        ...
        def forward_belief(belief, match_probs, ans):
            new_belief = copy.deepcopy(belief)

            for i in range(len(new_belief["cand_belief"])):
                # llh = LINGUISTIC_OBSERVATION_MODELS[
                #     'o={:d},m={:d}'.format(ans, is_matched[i])]
                # new_belief["cand_belief"][i].update_with_likelihood(
                #     llh, enable_low_thresh=False)
        ...
        def estimate_q_vec(belief, current_d):
            ...
            if current_d == planning_depth - 1:
                ...
                for i, match_prob in enumerate(belief["q_matching_prob"]):
                    ...
                    if i < num_obj and belief["cand_belief"][i].belief[0] == 1:
                    ...
                    # 2. update the belief
                    new_belief_yes = forward_belief(belief, match_prob, True)
                    new_belief_no = forward_belief(belief, match_prob, False)
    ...
    def _choose_optimal_action(self, q_vec):
        optimal_q, _ = torch.max(q_vec).item()
