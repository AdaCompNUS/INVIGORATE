...
class Ingress():
    ...
    def _filter_boxes(self, expr, boxes):
        query = expr
        rospy.loginfo("_filter_boxes: Input String: " + query)
    ...
    def _ground_query(self, expr, boxes):
        ...
        '''
        selection_orig_idx = self._filter_boxes(expr, boxes)
        ...
        query_goal = ingress_msgs.msg.BoxesRefexpQueryGoal(
            expr, np.array(selected_boxes).ravel(), selection_orig_idx, incorrect_idxs)
    ...
    def load_img_and_filter_bbox(self, image, bboxes, expr, true_names=None):
        '''
        Load images with predefined bounding box, returns filtered bboxes without actual groudning.
        @param image, cv2_img
        @param bbox, a list of bbox each in [top, left, width, height] format
        @param expr, user expression
        @param true_names, a list of true object names in bboxes. If the list is not None, the caption of bbox will be replaced by the true name
                           if it is far away from true name.
        @return filtered_bboxes, filtered bboxes
                filtered_captions, captions for those bboxes
                filtered_caption_scores, scores of captions for those bboxes
        '''
        # preprocess data
        # convert image to ros image
        img_msg = CvBridge().cv2_to_imgmsg(image, "rgb8")
        ...
        # filter bboxes according to expression
        selection_orig_idx = self._filter_boxes(expr, bboxes)
        filtered_bboxes = np.take(bboxes, selection_orig_idx, axis=0)
        filtered_captions = np.take(self._ungrounded_captions, selection_orig_idx, axis=0)
        filtered_caption_scores = np.take(self._ungrounded_caption_scores, selection_orig_idx, axis=0)

        return filtered_bboxes, filtered_captions, filtered_caption_scores
