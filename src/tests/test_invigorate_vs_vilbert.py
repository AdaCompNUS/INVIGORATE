...
def main():
    ...
    while not to_end:
        ...
        vilbert_target_prob = vilbert_grounding(img, bboxes, expr, num_obj)

        gt_target = raw_input("Enter idx for gt tgt:")
        gt_tgt_prob = generate_gt_tgt_prob(int(gt_target), num_obj)
        ...
        logger.info("--------------- loss:")
        logger.info("single-step: tgt: {}".format(invigorate_tgt_loss))
